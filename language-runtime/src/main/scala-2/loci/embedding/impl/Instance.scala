...
class Instance(val c: blackbox.Context) {
  ...
  object types {
    ...
    val instance = typeOf[language.Instance[_]]
    ...
    val connections = typeOf[embedding.Connections]
          ...
          case q"$prefix.$name[..$tpt](...$exprss)" =>
            ...
            val access =
              if (peer != NoType && result <:< types.per) {
                ...
                q"""new ${termNames.ROOTPKG}.loci.language.Instance.SubjectiveValue[$subjective, $remote] {
                  def to(remote: ${termNames.ROOTPKG}.loci.language.Remote[$remote]) = $access
                }"""
              }
              else
    ...
    expr match {
      ...
      case q"new { ..$earlydefs } with $inst[$tpt](...$exprss) { $self => ..$body }"
        ...
        val connect = (exprs
          ...
          getOrElse q"${termNames.ROOTPKG}.loci.embedding.Connections.empty")
