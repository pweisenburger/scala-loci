...
@multitier object ObjectModule {
  ...
  val v = on[loci.embedding.ObjectModule.Peer] { implicit! => 42 }

  object a {
    object b {
      val y = 0
      object o { val z = loci.embedding.ObjectModule.a.b.y }
      class C { val z = a.b.y }
    }
  }
}
...
class MacroExpansionSpec extends AnyFlatSpec with Matchers with NoLogging {
  ...
  it should "correctly lift types" in {
    val placedValuesClasses = new ClassLifting.`<placed values of loci.embedding.ClassLifting>` {
      lazy val $loci$sig = ""
      def $loci$sys$create = emptySystem(this)
    }
  ...
  it should "correctly compile nested multitier modules" in {
    new NestedMultitierModules.p.$loci$peer$loci$embedding$NestedMultitierModules$p$Peer {
      def $loci$sys$create = emptySystem(this)
    }.a.i should be (16)
    ...
    new NestedMultitierModules.q.$loci$peer$loci$embedding$NestedMultitierModules$q$Peer {
      def $loci$sys$create = emptySystem(this)
    }.a.i should be (0)
  ...
  it should "correctly compile derived multitier modules" in {
    new SubModule.`<placed values of loci.embedding.SubModule>` {
      def $loci$sys$create = emptySystem(this)
    }.buffer should be (null)

    new SubModule.$loci$peer$loci$embedding$SubModule$A {
      def $loci$sys$create = emptySystem(this)
    }.buffer should contain theSameElementsInOrderAs Seq(
      "super a.0", "super a.1", "sub a.0", "sub a.1")

    new SubModule.$loci$peer$loci$embedding$SubModule$B {
      def $loci$sys$create = emptySystem(this)
    }.buffer should contain theSameElementsInOrderAs Seq(
  ...
  it should "correctly instantiate peer " in {
    ...
    multitier start new language.Instance[mod.Peer](contexts.Immediate.global, separateMainThread = false) {
      @multitier object o {
