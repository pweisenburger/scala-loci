...
class SystemSpec extends AnyFlatSpec with Matchers with NoLogging:
  ...
  it should "handle remote access correctly" in:
    CompileTimeUtils.exposeSynthesizedMultitierValues(ServerClientApp)
    ...
    for seed <- 0 to 5 do
      ...
      Runtime.start(
        ...
        (context, connections, connected) =>
          val instance = new ServerClientApp.`<placed values on loci.runtime.ServerClientApp.Server>`:
            protected def $loci$sys$create = new System(this, None, false, context, connections, connected)
      ...
      Runtime.start(
        ...
        (context, connections, connected) =>
          val instance = new ServerClientApp.`<placed values on loci.runtime.ServerClientApp.Client>`:
            protected def $loci$sys$create = new System(this, None, false, context, connections, connected)
            override def `<placed id of loci.runtime.ServerClientApp>`() = 20
      ...
      Runtime.start(
        ...
        (context, connections, connected) =>
          val instance = new ServerClientApp.`<placed values on loci.runtime.ServerClientApp.Client>`:
            protected def $loci$sys$create = new System(this, None, false, context, connections, connected)
            override def `<placed id of loci.runtime.ServerClientApp>`() = 21
      ...
      val res0 = serverSystem.invokeRemoteAccess(
        (),
        ServerClientApp.$loci$placed$e6776799$0,
        ServerClientApp.$loci$peer$sig$Client,
        Seq.empty,
        requestResult = true)
      ...
      val res4 = client0System.invokeRemoteAccess(
        4,
        ServerClientApp.$loci$placed$e6776799$1,
        ServerClientApp.$loci$peer$sig$Server,
        Seq.empty,
