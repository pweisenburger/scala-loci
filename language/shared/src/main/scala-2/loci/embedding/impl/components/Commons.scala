...
class Commons[C <: blackbox.Context](val engine: Engine[C]) extends Component[C] {
  ...
  object trees {
    ...
    val marshallable = q"${names.root}.loci.transmitter.Marshallable.Resolution.marshallable"
    val nothingMarshallable = q"${names.root}.loci.transmitter.Marshallable.Resolution.nothing"
    val unitMarshallable = q"${names.root}.loci.transmitter.Marshallable.Resolution.unit"
